...
@export var single_p_scene: PackedScene
...
@onready var solo_button: Button = $Net/Options/SoloButton
...
func add_player(id):
	...
	player_instance.is_multi = is_multi
...
func _on_solo_button_pressed() -> void:
	is_multi = false
	var single_instance = single_p_scene.instantiate()
	add_child(single_instance)
	add_player(1)
